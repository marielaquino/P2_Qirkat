...
class Game {
    ...
    void process() {
        ...
        while (true) {
            ...
            if (_whiteIsManual) {
                whitePlayer = new Manual(this, WHITE);
            }
            if (!_whiteIsManual) {
                whitePlayer = new AI(this, WHITE);
            }
            if (_blackIsManual) {
                blackPlayer = new Manual(this, BLACK);

            }
            if (!_blackIsManual) {
                blackPlayer = new AI(this, BLACK);
            }
            ...
            while (_state != SETUP && !_board.gameOver()) {
                ...
                if (_board.whoseMove() == WHITE) {
                    move = whitePlayer.myMove();
                } else {
                    move = blackPlayer.myMove();
                }
    ...
    /** Return the other view of my game board. */
    Board boardUse() {
        return _board;
    }
    ...
    void doLoad(String[] operands) {
        try {
            ...
            _inputs.addSource(new ReaderSource(reader, true));
        } catch (IOException e) {
    ...
    void doMove(String[] operands) {
        Move result = Move.parseMove(operands[0]);
        _board.makeMove(result);
    }
    ...
    void doSet(String[] operands) {
        doClear(operands);
        _board.setPieces(operands[1],
                operands[0].toLowerCase().equals("white") ? WHITE : BLACK);
    ...
    void doDump(String[] unused) {
        ...
        out.format("===\n");
        out.format(_board.toString());
        out.format("\n===");
        _reporter.moveMsg(out.toString());
    ...
    /** Holds the white player in the global frame. */
    private Player whitePlayer;
    /** Holds the black player in the global frame. */
