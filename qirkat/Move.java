...
class Move {
    ...
    char jumpedRow() {
        if (this.row1() == this.row0()) {
            return this.row1();
        } else {
            return (char) ((this.row0() + this.row1()) / 2);
        }
    }
    ...
    char jumpedCol() {
        if (this.col0() == this.col1()) {
            return this.col0();
        } else {
            return (char) ((this.col0() + this.col1()) / 2);
    ...
    private void toString(Formatter out) {
        if (isVestigial()) {
            out.format(" ", this.col0(), this.row0());
        }
        out.format("%c%c", this.col0(), this.row0());
        Move temp = this;
        while (temp != null) {
            out.format("-%c%c", temp.col1(), temp.row1());
            temp = temp._nextJump;
        }
